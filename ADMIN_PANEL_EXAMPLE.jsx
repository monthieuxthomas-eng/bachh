// Admin Panel Component - Pour la validation des tickets
// À intégrer dans votre système d'admin
...
const AdminPanel = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribePending = onSnapshot(pendingQuery, (snapshot) => {
      setTickets((prev) => ({
        ...prev,
        pending: snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
      }));
    });

    // Écouter les tickets minted
    ...
    const unsubscribeMinted = onSnapshot(mintedQuery, (snapshot) => {
      setTickets((prev) => ({
        ...prev,
        minted: snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
      }));
    });

    // Écouter les tickets failed
    ...
    const unsubscribeFailed = onSnapshot(failedQuery, (snapshot) => {
      setTickets((prev) => ({
        ...prev,
        failed: snapshot.docs.map((doc) => ({ id: doc.id, ...doc.data() })),
      }));
      setLoading(false);
    });
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="text-4xl font-bold mb-8 text-green-900">Panel Admin - Tickets</h1>
                ...
                <div className="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-sm">
                      {ticket.userAddress.substring(0, 10)}...{ticket.userAddress.substring(-8)}
                    </p>
                  </div>
                  <div>
              ...
              <div key={ticket.id} className="border-2 border-green-300 rounded-lg p-4 bg-green-50">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-sm">{ticket.transactionHash.substring(0, 10)}...</p>
                  </div>
                </div>
              </div>
            ))}
